...
# The following tools have to be installed: sed, awk, wget.
#
# syntax:
...
if [ "x$wgetpath" = "x" ]; then
    ...
    echo "  wget for Windows can be downloaded from:"
    echo "  https://eternallybored.org/misc/wget/"
    echo "  On debian based Linux systems it can be installed with:"
...
do
    ...
    if [ "$src" != "index" ]; then
        ...
        # in temp_file. Only include *.dsk files.
        contents="-"
        date="-"
        for dsk_file in `ls ${target_dir}/$1 -I *.log -I *.TXT`
        do
            result=`echo $dsk_file | sed -n -e "s/^\(.*\.[dD][sS][kK]\)$/\1/p"`
            if [ "x$result" != "x" ]; then
